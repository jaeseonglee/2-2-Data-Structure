...
public class BST <Key extends Comparable<Key>, Value>{
	...
	public Key kthSmallest(int k) {	// K번째 작은 값을 찾아 반환하는 kthSmallest메소드
		arr = new int[size(root)];	// 배열을 트리의 노드 갯수로 초기화 해준다
		num = 0;					// 메소드 호출마다 num 을 0으로 초기화
		kthSmallest( root);			// kthSmallest 오버로딩, 루트 노드 전달
		Object temp = (Object)arr[k];	//Object 변수 temp에 k번째 배열 값 저장
		return (Key)temp;		// temp를 Key로 반환
	}
	
	private void kthSmallest( Node n) {
		if(n != null) {		// 재귀호출의 형태를 가진다	
			kthSmallest( n.getLeft());	// 중위순회로 먼저 왼쪽 자식을 확인
			arr[num] = (int) n.getKey();	// 현재의 Key값을 배열에 저장
			num++;		// num 증가
			kthSmallest( n.getRight());	// 왼쪽자식과 루트를 확인후 오른쪽 자식 확인
		}					
	}
}
